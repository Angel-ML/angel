...
在寻找分裂点的过程中，需要建立梯度直方图。对于连续特征的数据集，已有的方法需要读取每个训练数据的每个特征，找到梯度直方图中对应的bucket，这种方法的计算复杂度是O(N*M)，其中N为训练数据数量，M为特征数量。对于目前工业界的数据集，N达到亿级别，M达到千万甚至亿级别，传统的方法对于这样的高维度数据集，计算复杂度过高。
...
假设训练数据的非0特征数量为k，那么这种方法的计算复杂度是O(N*k+M)，由于k<<M，与已有的方法相比，大大降低了计算复杂度。实验结果显示，这种针对稀疏数据的方法能够带来1-2个数量级的速度提升。
