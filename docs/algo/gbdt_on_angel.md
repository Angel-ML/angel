...
我们注意到，绝大多数高维数据集是稀疏的，也就是只有少数特征是非0的特征值，大多数特征是0。对应到我们的存储方式，我们对于稀疏数据集采用稀疏的存储格式，只包含非0的特征和特征值。因而，这种数据集的特征引起了一个问题，我们能够利用数据的稀疏性来加速梯度直方图的建立？
...
2. **筛选非0的特征：** 依次处理每个训练数据，从稀疏存储的格式中取出非0的特征和特征值。对于一个非0特征的梯度，将其从zero bucket中取出，并放入其对应的bucket中。
