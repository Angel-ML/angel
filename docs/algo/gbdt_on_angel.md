...
3. **两阶段树分裂算法：** 在寻找最佳分裂点时，可以在多个PS节点上并行进行，只需要将局部最优分裂点返回给Worker，通信开销几乎可以忽略不计。

整体来看，Angel的PS优势，使得它在这个算法上，性能远远超越了Spark版本的GBDT实现，而和MPI版本的xgBoost也略胜一筹。

## 4. 建立梯度直方图

在寻找分裂点的过程中，需要建立梯度直方图。对于连续特征的数据集，已有的方法需要读取每个训练数据的每个特征，找到梯度直方图中对应的bucket，这种方法的计算复杂度是 **O(N*M)** ，其中N为训练数据数量，M为特征数量。对于目前工业界的数据集，N达到亿级别，M达到千万甚至亿级别，传统的方法对于这样的高维度数据集，计算复杂度过高。

我们注意到，绝大多数高维数据集是稀疏的，也就是只有少数特征是非0的特征值，大多数特征是0。对应到我们的存储方式，我们对于稀疏数据集采用稀疏的存储格式，只包含非0的特征和特征值。这种数据集的特征因而引起了一个问题，我们能够利用数据的稀疏性来加速梯度直方图的建立？

![GBDT Build Histgram](../img/sparse_build_hist.png)

我们用一种逆向思维作为切入点，既然大多数特征值是0，那么我们可以事先假设所有特征都是0，而把非0的特征值作为异常情况。基于这种思路，我们提出了一种gather-and-filter的策略（如上图所示）：

1. **所有数据放入zero bucket** 每个特征的梯度直方图中，有一个bucket包含特征值0，我们将这个bucket叫做zero bucket，并将所有训练数据的梯度放入zero bucket中累加。

2. **筛选非0的特征：** 依次处理每个训练数据，从稀疏存储的格式中取出非0的特征和特征值。对于一个非0的item，将其从zero bucket中取出，并放入其对应的bucket中。

假设训练数据的非0特征数量为k，那么这种方法的计算复杂度是 **O(N*k+M)** ，由于k<<M，与已有的方法相比，大大降低了计算复杂度。实验结果显示，这种针对稀疏数据的方法能够带来1-2个数量级的速度提升。
