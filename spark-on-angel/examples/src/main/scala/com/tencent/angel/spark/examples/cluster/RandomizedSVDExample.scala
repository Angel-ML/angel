...
object RandomizedSVDExample {
  ...
  def main(args: Array[String]) = {
    ...
    val outputS = params.getOrElse("outputS", null)
    val outputV = params.getOrElse("outputV", null)
    val outputU = params.getOrElse("outputU", null)
    ...
    //算法参数
    val k = params.getOrElse("K", "3").toInt
    val rCond = params.getOrElse(RCOND, "1e-9").toDouble

    //parameters for randomized
    val iterationNormalizer = params.getOrElse(ITERATION_NORMALIZER, QR)
    val qOverSample = params.getOrElse(Q_OVERSAMPLE, "3").toInt
    val numIteration = params.getOrElse(NUM_ITERATION, AUTO)
