...
class VectorOps {
  ...
  def equal(v1: PSVector, v2: PSVector): Boolean = {
    ...
    val res = psfGet(v1.poolId, new Equal(v1.poolId, v1.id, v2.id)).asInstanceOf[ScalarAggrResult].getResult
    res == 1.0
  }
  ...
  def asum(x: PSVector): Double = {
    x.assertValid()
    psfGet(x.poolId, new Asum(x.poolId, x.id))
      .asInstanceOf[ScalarAggrResult].getResult
  }
  ...
  def amax(x: PSVector): Double = {
    x.assertValid()
    psfGet(x.poolId, new Amax(x.poolId, x.id))
      .asInstanceOf[ScalarAggrResult].getResult
  }
  ...
  def amin(x: PSVector): Double = {
    x.assertValid()
    psfGet(x.poolId, new Amin(x.poolId, x.id))
      .asInstanceOf[ScalarAggrResult].getResult
  }
}
