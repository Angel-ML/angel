...
class NeighborTable(@BeanProperty val param: Param) extends Serializable {
  ...
  def initNeighbor(data: RDD[(Int, Int)]): NeighborTable = initNeighbor(data, param)

  def initNeighbor(data: RDD[(Int, Int)], param: Param): NeighborTable = {
  ...
  def initLongNeighbor(data: RDD[(Long, Array[Long])]): NeighborTable = {
    // Neighbor table : a (1, maxIndex + 1) dimension matrix
  ...
  def initLongNeighbors(psMatrix: PSMatrix, pairs: Seq[(Long, Array[Long])]) = {
