...
class LINEModel(dataset: Dataset[_], embeddingDim: Int, negativeNum: Int, stepSize: Float, order: Int,
  ...
  /**
    ...
    */
  def saveRemappingResult(stringsWithIndex: RDD[(String, Long)]) = {
  ...
  /**
    ...
    * @param epoch
    */
  def checkpointAndSaveIfNeed(epoch: Int) = {
    var startTs = 0L
  ...
  def save(outPath:String, epochNum:Int, saveMeta: Boolean) = {
...
class LINEWithWightModel(dataset: Dataset[_], embeddingDim: Int, negativeNum: Int, stepSize: Float, order: Int,
  ...
  def sgdForBatch(batchSize: Int,
    ...
    //for(i <- 0 until srcNodes.length) {
    //  edgeCounter.addTo(new Edge(srcNodes(i), destNodes(i)), 1)
    //}

    //println("counter size = " + edgeCounter.size())
