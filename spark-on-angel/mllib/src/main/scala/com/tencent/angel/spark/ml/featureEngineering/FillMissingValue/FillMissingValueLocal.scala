...
object FillMissingValueLocal {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mode = "local"
    val input = "data/mlDataTest/FillMissingValue_data_test.txt"
    val output = "data/output/output"
    val partitionNum = 1
    val sampleRate = 1
    val sep = ","
    // feature config to fill the missing value
    val fillConfPath = "data/mlDataTest/FillMissingValueConf.txt"
    // this path is for save the fill method for each specified feature
    val fillStatPath = "data/output/fillStatPath"

    val sc = start(mode)
    val inputDF = DataLoader.loadTableWithNull(sc, input, partitionNum, sampleRate, null, sep)
