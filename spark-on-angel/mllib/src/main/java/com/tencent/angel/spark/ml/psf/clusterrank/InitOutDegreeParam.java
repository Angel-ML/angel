...
public class InitOutDegreeParam extends UpdateParam {

    private Long2IntMap nodeIdToOutDegree;

    public InitOutDegreeParam(int matrixId, Long2IntMap nodeTooutDegree) {
        super(matrixId);
        this.nodeIdToOutDegree = nodeTooutDegree;
    }

    @Override
    public List<PartitionUpdateParam> split() {
        long[] nodeIds = nodeIdToOutDegree.keySet().toLongArray();
        ...
        while (nodeIndex < nodeIds.length || partIndex < partitions.size()) {
            ...
            if (length > 0) {
                partParams.add(new PartInitOutDegreeParam(matrixId,
                        partitions.get(partIndex), nodeIdToOutDegree, nodeIds, nodeIndex - length,
                        nodeIndex));
            }
            partIndex++;
        }
