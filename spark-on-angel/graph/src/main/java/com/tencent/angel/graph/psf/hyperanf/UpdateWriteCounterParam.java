...
public class UpdateWriteCounterParam extends UpdateParam {
  ...
  public UpdateWriteCounterParam(int matrixId, Long2ObjectOpenHashMap<HyperLogLogPlus> updates, int p, int sp, long seed
  ) {
    super(matrixId);
  ...
  public List<PartitionUpdateParam> split() {
    ...
    for (int i = 0; i < parts.length; i++) {
      if (splits[i] != null && splits[i].size() > 0) {
        partParams.add(new UpdateWriteCounterPartParam(matrixId, parts[i], splits[i], p, sp, seed));
      }
    }
