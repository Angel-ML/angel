...
public class GetNeighborAliasTableParam extends GetParam {
    ...
    public GetNeighborAliasTableParam(int matrixId, long[] nodeIds, int[] count) {
        super(matrixId);
    ...
    public GetNeighborAliasTableParam() {
        this(-1, null, null);
    }
    ...
    public List<PartitionGetParam> split() {
        ...
        while (nodeIndex < nodeIds.length || partIndex < partitions.size()) {
            ...
            if (length > 0) {
                PartGetNeighborAliasTableParam param = new PartGetNeighborAliasTableParam(matrixId,
                        partitions.get(partIndex), count, nodeIds, nodeIndex - length, nodeIndex);
                partParams.add(param);
            }
            partIndex++;
        }
