...
package com.tencent.angel.graph.psf.embedding
...
class PartitionWrapper(val part: RowBasedPartition, val partDim: Int, val order: Int) {
  ...
  def dot(inputVectorId: Int, outputVectorId: Int): Float = {
    ...
    val inputRow = ServerRowUtils.getVector(part.getRow(inRowId)).getStorage.asInstanceOf[IntFloatDenseVectorStorage].getValues
    val outputRow = ServerRowUtils.getVector(part.getRow(outRowId)).getStorage.asInstanceOf[IntFloatDenseVectorStorage].getValues
  ...
  def axpy(a: Float, nodeId: Int, isInputVec: Boolean, y: Array[Float]): Unit = {
    ...
    val row = ServerRowUtils.getVector(part.getRow(rowId)).getStorage.asInstanceOf[IntFloatDenseVectorStorage].getValues
    for (i <- 0 until partDim)
      y(i) += a * row(colId + i)
  }
