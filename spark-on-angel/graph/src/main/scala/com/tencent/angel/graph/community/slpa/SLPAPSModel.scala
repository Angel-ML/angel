...
class SLPAPSModel (var comInMsgs: PSMatrix,
...
object SLPAPSModel {
  ...
  def apply(modelContext: ModelContext, data: RDD[Long],
            useBalancePartition: Boolean, balancePartitionPercent: Float): SLPAPSModel = {
    ...
    new SLPAPSModel(comInPsMatrix, comOutPsMatrix)
