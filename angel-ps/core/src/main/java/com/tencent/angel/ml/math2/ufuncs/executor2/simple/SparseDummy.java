...
public class SparseDummy {
  ...
  private static String union = "{\n" +
		  ...
		  "		v = CommonFuncs.getInstance().otherVsDummy(v1, v2, newStorage,op);\n" +
		  ...
		  "		&itypeLow capacity = 1 << (32 - Integer.numberOfLeadingZeros((int) (v1.size() / 0.75)));\n" +
			...
			for (int q = 0; q < names.length; q++) {
			  for (int i = 0; i < itypes.length; i++) {
				for (int j = 0; j < dtypes.length; j++) {
				  ...
				  if (itypes[i] == "Long") {
					body = "{\n" +
							"throw new AngelException(\"Vector is not support! \");" +
							"}\n\n";
				  } else {
					if (names[q] == "union") {
					  body = union;
