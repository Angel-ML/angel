...
public class GradLogLoss extends Binary {
  public GradLogLoss(boolean inplace) {
    setInplace(inplace);
  ...
  @Override public double apply(double ele1, double ele2) {
    return -ele2 / (1 + Math.exp(ele1 * ele2));
  }
  ...
  @Override public float apply(float ele1, float ele2) {
    return (float) (-ele2 / (1 + Math.exp(ele1 * ele2)));
  }
  ...
  @Override public long apply(long ele1, long ele2) {
    return (long) (-ele2 / (1 + Math.exp(ele1 * ele2)));
  }
