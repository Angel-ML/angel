package com.tencent.angel.common.collections;

import com.tencent.angel.common.ByteBufSerdeUtils;
import io.netty.buffer.ByteBuf;
import java.util.ArrayList;
import java.util.List;

public class DynamicIntArray extends DynamicArray {

  private List<int[]> batchs;
  private int[] currentBatch;
  private int[] data;

  public DynamicIntArray(int size) {
    if (size > 0) {
      batchs = new ArrayList<>(size / batchSize + 1);
    } else {
      batchs = new ArrayList<>(defaultBatchNum);
    }

    batchs.add(new int[batchSize]);
    currentBatch = batchs.get(0);
  }

  public DynamicIntArray() {
  }

  public void add(int value) {
    currentBatch[currentBatchPos] = value;
    currentBatchPos++;

    if (currentBatchPos == batchSize) {
      currentBatchPos = 0;
      currentBatchIndex++;
      if (currentBatchIndex < batchs.size()) {
        currentBatch = batchs.get(currentBatchIndex);
      } else {
        batchs.add(new int[batchSize]);
        currentBatch = batchs.get(currentBatchIndex);
      }
    }
  }

  public List<int[]> getBatchs() {
    return batchs;
  }

  public int[] getCurrentBatch() {
    return currentBatch;
  }

  public int[] getData() {
    if (data == null) {
      data = new int[size()];
      // [0, currentBatchIndex - 1] batchs
      int offset = 0;
      for (int i = 0; i < currentBatchIndex; i++) {
        int[] batch = batchs.get(i);
        System.arraycopy(batch, 0, data, offset, batch.length);
        offset += batch.length;
      }

      // Last batch
      for (int i = 0; i < currentBatchPos; i++) {
        System.arraycopy(currentBatch, 0, data, offset, currentBatchPos);
      }
    }
    return data;
  }

  @Override
  public void serialize(ByteBuf out) {
    ByteBufSerdeUtils.serializeInt(out, batchSize * currentBatchIndex + currentBatchPos);

    // [0, currentBatchIndex - 1] batchs
    for (int i = 0; i < currentBatchIndex; i++) {
      int[] batch = batchs.get(i);
      for (int j = 0; j < batch.length; j++) {
        ByteBufSerdeUtils.serializeInt(out, batch[j]);
      }
    }

    // Last batch
    for (int i = 0; i < currentBatchPos; i++) {
      ByteBufSerdeUtils.serializeInt(out, currentBatch[i]);
    }
  }

  @Override
  public void deserialize(ByteBuf in) {
    data = ByteBufSerdeUtils.deserializeInts(in);
  }

  @Override
  public int bufferLen() {
    return ByteBufSerdeUtils.serializedIntLen(0) * (batchSize * currentBatchIndex
        + currentBatchPos);
  }
}
