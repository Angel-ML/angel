...
public class SigmoidWithDropout extends Unary {

  private double threshold = Math.log(Double.MAX_VALUE);
  private double proba;

  public SigmoidWithDropout(boolean inplace, double proba) {
  ...
  public double apply(double elem) {
    if (proba == 1) {
      ...
      if (elem > threshold) {
        ...
        return (1.0 / (1.0 + Math.exp(-elem))) / (1 - proba);
      }
    } else {
      return 0;
    }
  ...
  public float apply(float elem) {
    if (proba == 1) {
      ...
      if (elem > threshold) {
        ...
        return (float) ((1.0 / (1.0 + Math.exp(-elem))) / (1 - proba));
      }
    } else {
      return 0;
    }
  ...
  public long apply(long elem) {
    if (proba == 1) {
      ...
      if (elem > threshold) {
        ...
        return (long) ((1.0 / (1.0 + Math.exp(-elem))) / (1 - proba));
      }
    } else {
      return 0;
    }
  ...
  public int apply(int elem) {
    if (proba == 1) {
      ...
      if (elem > threshold) {
        ...
        return (int) ((1.0 / (1.0 + Math.exp(-elem))) / (1 - proba));
      }
    } else {
      return 0;
    }
