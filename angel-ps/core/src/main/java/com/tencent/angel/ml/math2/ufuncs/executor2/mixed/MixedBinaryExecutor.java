...
public class MixedBinaryExecutor {
  public static Vector apply(Vector v1, Vector v2, Binary op) {
	Vector v;
	if (op.getOpType() == OpType.UNION) {
	  v = MixedBinaryUnionExecutor.getInstance().apply(v1, v2, op);
	} else if (op.getOpType() == OpType.INTERSECTION) {
	  v = MixedBinaryIntersectionExecutor.getInstance().apply(v1, v2, op);
	} else {
	  v = MixedBinaryAllExecutor.getInstance().apply(v1, v2, op);
	}
	return v;
  }
}
