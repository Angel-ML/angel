...
public class HuberLoss extends Binary {

  private double delta;
  private double constant;

  public HuberLoss(double delta, boolean inplace) {
    setInplace(inplace);
  ...
  public float apply(float ele1, float ele2) {
    ...
    if (diff <= delta) {
      ...
      return (float) (delta * diff - constant);
    }
  }
  ...
  public long apply(long ele1, long ele2) {
    ...
    if (diff <= delta) {
      return (long) (0.5 * diff * diff);
    } else {
      return (long) (delta * diff - constant);
    }
  }
