...
public class SortedSparse {
  ...
  private static String union = "{\n" +
		  "Vector v = null;\n" +
		  "if (op.isKeepStorage()) {\n" +
		  "  v = CommonFuncs.getInstance().sortedVsSparse1(v1, v2, newStorage, op);\n" +
		  "} else {\n" +
		  "  CommonFuncs.getInstance().replaceOperation(v1, newStorage);\n" +
		  "  v = CommonFuncs.getInstance().otherVsSparse(v1, v2, newStorage, op);\n" +
		  "}\n" +
		  inplaceOrNot +
		  "}\n\n";
  ...
  private static String intersection = "{\n" +
		  ...
		  "  v = CommonFuncs.getInstance().otherVsSparse(v1, v2, newStorage, op);\n" +
		  "} else {\n" +
		  "  v = CommonFuncs.getInstance().sortedVsSparse2(v1, v2, newStorage, op);\n" +
		  "}\n" +
		  inplaceOrNot +
		  "}\n\n";
