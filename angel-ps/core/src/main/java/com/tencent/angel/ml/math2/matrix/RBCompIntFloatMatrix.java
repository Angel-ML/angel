...
public class RBCompIntFloatMatrix extends RowBasedMatrix<CompIntFloatVector> {
    ...
    if (op.isInplace()) {
      ...
      for (int i = 0; i < rows.length; i++) {
        ...
        if (null == ((RowBasedMatrix) other).rows[i])
          ((RowBasedMatrix) other).initEmpty(i);
        outRows[i] =
          (CompIntFloatVector) BinaryExecutor.apply(rows[i], ((RowBasedMatrix) other).rows[i], op);
      }
  ...
  @Override public void setRow(int idx, CompIntFloatVector v) {
    assert cols == v.getDim();
    rows[idx] = v;
  }

  @Override public void setRows(CompIntFloatVector[] rows) {
