...
public class SimpleBinaryAllExecutor {
  ...
  private static String IntKeyDummyFunc = "{\n" +
		  ...
		  "if (v1.isDense()) {\n" +
		  "  return DenseDummy.getInstance().all(v1, v2, newStorage, op);\n" +
		  "} else ";

  private static String LongKeyDummyFunc =
		  "if (v1.isSparse()) {\n" +
				  "  return SparseDummy.getInstance().all(v1, v2, newStorage, op);\n" +
				  "} else {\n" +
				  "  return SortedDummy.getInstance().all(v1, v2, newStorage, op);\n" +
				  "}\n" +
				  "}\n\n";
  ...
  private static String IntKeyFunc = "{\n" +
		  ...
		  "  return DenseDense.getInstance().all(v1, v2, newStorage, op);\n" +
		  "} else if (v1.isDense() && v2.isSparse()) {\n" +
		  "  return DenseSparse.getInstance().all(v1, v2, newStorage, op);\n" +
		  "} else if (v1.isDense() && v2.isSorted()) {\n" +
		  "  return DenseSorted.getInstance().all(v1, v2, newStorage, op);\n" +
		  "} else if (v1.isSparse() && v2.isDense()) {\n" +
		  "  return SparseDense.getInstance().all(v1, v2, newStorage, op);\n" +
		  "} else if (v1.isSorted() && v2.isDense()) {\n" +
		  "  return SortedDense.getInstance().all(v1, v2, newStorage, op);\n" +
		  "} else ";

  private static String LongKeyFunc =
		  "if (v1.isSparse() && v2.isSparse()) {\n" +
				  "  return SparseSparse.getInstance().all(v1, v2, newStorage, op);\n" +
				  "}else if (v1.isSparse() && v2.isSorted()) {\n" +
				  "  return SparseSorted.getInstance().all(v1, v2, newStorage, op);\n" +
				  "} else if (v1.isSorted() && v2.isSparse()) {\n" +
				  "  return SortedSparse.getInstance().all(v1, v2, newStorage, op);\n" +
				  "} else if (v1.isSorted() && v2.isSorted()) {\n" +
				  "  return SortedSorted.getInstance().all(v1, v2, newStorage, op);\n" +
				  "} else {\n" +
