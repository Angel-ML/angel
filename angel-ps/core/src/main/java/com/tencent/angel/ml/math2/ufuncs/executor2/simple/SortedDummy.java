...
public class SortedDummy {
  private volatile static DummyStorageOperation executor;
  ...
  private static String union = "{\n" +
		  "Vector v = null;\n" +
		  "if (op.isKeepStorage()) {\n" +
		  "  v = CommonFuncs.getInstance().sortedVsDummy1(v1, v2, newStorage, op);\n" +
		  "} else {\n" +
		  "  v = CommonFuncs.getInstance().sortedVsDummy2(v1, v2, newStorage, op);\n" +
		  "}\n" +
		  inplaceOrNot +
		  "}\n\n";
  ...
  public static DummyStorageOperation getInstance() {
	if (executor == null) {
	  synchronized (DummyStorageOperation.class) {
		if (executor == null) {
		  try {
			  ...
			  for (int i = 0; i < itypes.length; i++) {
				for (int j = 0; j < dtypes.length; j++) {
				  declare = declTemplate.replaceAll("&ReturnType", "Vector")
						  ...
						  .replaceAll("&Parameters", "&itpye&dtype1Vector v1, &itpyeDummyVector v2" +
								  ", &itpye&dtype1VectorStorage newStorage, Binary op")
				  ...
				  if (names[q] == "union") {
					...
					if (dtypes[j] == "Double" || dtypes[j] == "Float") {
					  ...
					  replace = "throw new AngelException(\"div by 0L!\")";
					} else {
					  replace = "throw new AngelException(\"div by 0!\")";
					}
					body = all;
				  }
				  ...
				  method = bodyTemplate.replaceAll("&Declaration", declare)
						  ...
						  .replaceAll("&itpyeLow", itypes[i].toLowerCase())
			...
			executor = (DummyStorageOperation) ee.createFastClassBodyEvaluator(
					new Scanner(null, new StringReader(
							template)),
					DummyStorageOperation.class,
					(ClassLoader) null
			);
		  } catch (Exception e) {
