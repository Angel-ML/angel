...
public class RBCompLongLongMatrix extends RowBasedMatrix<CompLongLongVector> {
    ...
    if (op.isInplace()) {
      ...
      for (int i = 0; i < rows.length; i++) {
        ...
        if (null == ((RowBasedMatrix) other).rows[i])
          ((RowBasedMatrix) other).initEmpty(i);
        outRows[i] =
          (CompLongLongVector) BinaryExecutor.apply(rows[i], ((RowBasedMatrix) other).rows[i], op);
      }
  ...
  @Override public void setRow(int idx, CompLongLongVector v) {
    assert cols == v.getDim();
    rows[idx] = v;
  }

  @Override public void setRows(CompLongLongVector[] rows) {
