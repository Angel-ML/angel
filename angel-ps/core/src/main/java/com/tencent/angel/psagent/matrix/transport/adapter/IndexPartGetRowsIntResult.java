...
public class IndexPartGetRowsIntResult extends IndexPartGetRowsResult {
  private volatile Map<Integer, int[]> values;

  public IndexPartGetRowsIntResult(PartitionKey partKey, List<Integer> rowIds, IndicesView indices,
    Map<Integer, int[]> values) {
    super(partKey, rowIds, indices);
    this.values = values;
  }

  public IndexPartGetRowsIntResult() {
  ...
  @Override public void deserializeData(ByteBuf buf) {
    ...
    for (int i = 0; i < rowNum; i++) {
      ...
      int[] colVals = new int[colNum];
      for (int j = 0; j < colNum; j++) {
        colVals[j] = buf.readInt();
      }
