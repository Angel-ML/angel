...
public class FMLocalExample {
  ...
  public void setConf(int mode) {
    ...
    if (mode == 1) {  // train mode
      ...
      conf.set(AngelConf.ANGEL_SAVE_MODEL_PATH, LOCAL_FS + TMP_PATH + "/model/FM");
    } else if (mode == 2) { // incTrain mode
      conf.set(AngelConf.ANGEL_ACTION_TYPE, "inctrain");
      conf.set(AngelConf.ANGEL_TRAIN_DATA_PATH, trainInput);
      conf.set(AngelConf.ANGEL_LOAD_MODEL_PATH, LOCAL_FS + TMP_PATH + "/model/FM");
      conf.set(AngelConf.ANGEL_SAVE_MODEL_PATH, LOCAL_FS + TMP_PATH + "/model/FM-inc");
      ...
      conf.set(AngelConf.ANGEL_LOAD_MODEL_PATH, LOCAL_FS + TMP_PATH + "/model/FM");
      conf.set(AngelConf.ANGEL_PREDICT_PATH, LOCAL_FS + TMP_PATH + "/com/tencent/angel/ml/predict/FM");
    }
    ...
    conf.setLong(MLCoreConf.ML_RANK_NUM(), 4);

    // Set model class
    conf.set(MLCoreConf.ML_MODEL_CLASS_NAME(), CLASSBASE + "FactorizationMachines");
  }
  ...
  public void train() {
    ...
    try {
      ...
      LOG.error("run FMLocalExample:train failed.", e);
      throw e;
    }
  ...
  public void incTrain() {
    ...
    try {
      ...
      LOG.error("run FMLocalExample:incTrain failed.", e);
      throw e;
    }
  ...
  public void predict() {
    ...
    try {
      ...
      LOG.error("run FMLocalExample:predict failed.", e);
      throw e;
    }
  }

  public static void main(String[] args) throws Exception {
    FMLocalExample example = new FMLocalExample();
