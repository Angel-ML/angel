...
public class PartGetFloatArrayAttrsResult extends PartitionGetResult {

  private int partId;
  private Long2ObjectOpenHashMap<float[]> nodeIdToContents;

  public PartGetFloatArrayAttrsResult(int partId,
      Long2ObjectOpenHashMap<float[]> nodeIdToContents) {
    this.partId = partId;
  ...
  public void serialize(ByteBuf output) {
    ...
    for (Long2ObjectOpenHashMap.Entry<float[]> entry : nodeIdToContents.long2ObjectEntrySet()) {
  ...
  public void deserialize(ByteBuf input) {
    ...
    for (int i = 0; i < size; i++) {
      nodeIdToContents.put(ByteBufSerdeUtils.deserializeLong(input),
          ByteBufSerdeUtils.deserializeFloats(input));
    }
  }
  ...
  public int bufferLen() {
    ...
    for (Long2ObjectOpenHashMap.Entry<float[]> entry : nodeIdToContents.long2ObjectEntrySet()) {
      ...
      len += ByteBufSerdeUtils.serializedFloatsLen(entry.getValue());
    }
    return len;
  }
}