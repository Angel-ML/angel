...
public class PartGetIntArrayAttrsResult extends PartitionGetResult {

  private int partId;
  private Long2ObjectOpenHashMap<int[]> nodeIdToContents;

  public PartGetIntArrayAttrsResult(int partId, Long2ObjectOpenHashMap<int[]> nodeIdToContents) {
    this.partId = partId;
    this.nodeIdToContents = nodeIdToContents;
  }

  public PartGetIntArrayAttrsResult() {
    this(-1, null);
  }
  ...
  public void serialize(ByteBuf output) {
    ...
    for (Long2ObjectOpenHashMap.Entry<int[]> entry : nodeIdToContents.long2ObjectEntrySet()) {
      ByteBufSerdeUtils.serializeLong(output, entry.getLongKey());
      ByteBufSerdeUtils.serializeInts(output, entry.getValue());
    }
  }
  ...
  public void deserialize(ByteBuf input) {
    ...
    for (int i = 0; i < size; i++) {
      nodeIdToContents.put(ByteBufSerdeUtils.deserializeLong(input),
          ByteBufSerdeUtils.deserializeInts(input));
    }
  }
  ...
  public int bufferLen() {
    ...
    for (Long2ObjectOpenHashMap.Entry<int[]> entry : nodeIdToContents.long2ObjectEntrySet()) {
      ...
      len += ByteBufSerdeUtils.serializedIntsLen(entry.getValue());
    }
    return len;
  }
}