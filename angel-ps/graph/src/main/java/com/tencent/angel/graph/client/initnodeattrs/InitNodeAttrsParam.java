...
public class InitNodeAttrsParam extends UpdateParam {
    ...
    public InitNodeAttrsParam(int matrixId, Long2ObjectMap<float[]> nodeIdToAttrIndices) {
        super(matrixId);
    ...
    public List<PartitionUpdateParam> split() {
        long[] nodeIds = nodeIdToAttrIndices.keySet().toLongArray();
        ...
        while (nodeIndex < nodeIds.length || partIndex < partitions.size()) {
            ...
            if (length > 0) {
                partParams.add(new PartInitNodeAttrsParam(matrixId,
                        partitions.get(partIndex), nodeIdToAttrIndices, nodeIds, nodeIndex - length,
                        nodeIndex));
            }
            partIndex++;
        }
