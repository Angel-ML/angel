...
class NullUnary(var isInplace:Boolean) extends Unary {
  override def apply(v1: DenseDoubleVector):DenseDoubleVector = {if (isInplace) v1 else v1.clone() }

  override def apply(v1: SparseDoubleVector):SparseDoubleVector = {if (isInplace) v1 else v1.clone() }

  override def apply(v1: SparseLongKeyDoubleVector):SparseLongKeyDoubleVector = {if (isInplace) v1 else v1.clone() }

  override def apply(v1: DenseFloatVector):DenseFloatVector = {if (isInplace) v1 else v1.clone() }

  override def apply(v1: SparseFloatVector):SparseFloatVector = {if (isInplace) v1 else v1.clone() }

  override def apply(v1: SparseLongKeyFloatVector):SparseLongKeyFloatVector = {if (isInplace) v1 else v1.clone() }
}
