...
public class RangeBitSetGetRowFunc extends GetRowFunc {
  ...
  public static class RangeBitSetPartitionGetParam extends PartitionGetParam {
    ...
    public RangeBitSetPartitionGetParam(int matrixId, PartitionKey partKey,
                                        int from, int to) {
  ...
  public GetResult merge(List<PartitionGetResult> partResults) {
    ...
    List<RangeBitSetGetRowResult.RangeBitSetPartitionGetRowResult>
            partResultList = new ArrayList<>(size);
    for (int i = 0; i < size; i++) {
      partResultList.add((RangeBitSetGetRowResult
              .RangeBitSetPartitionGetRowResult)partResults.get(i));
    }
    if (size == 1) {
