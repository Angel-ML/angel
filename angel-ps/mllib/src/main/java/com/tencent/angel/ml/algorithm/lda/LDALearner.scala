...
import com.tencent.angel.ml.algorithm.lda.structures.S2STraverseMap
import com.tencent.angel.ml.algorithm.utils.MathUtils
import com.tencent.angel.ml.math.vector.{DenseIntVector, SparseIntSortedVector}
...
class LDALearner(ctx: TaskContext,
  ...
  def initOneBatch(queue: ConcurrentLinkedQueue[TokensOneWord], batchNum: Int): Unit = {
    LOG.info("Start mini batch " + batchNum)
    ...
    lDAModel.tMat.clock().get()
    lDAModel.wtMat.clock().get()
    LOG.info("Finish init batch " + batchNum)
  }
  ...
  def trainOneBatch(batchNum: Int, rowIndex: RowIndex): Unit = {
    ...
    LOG.info("Finish batch " + batchNum)
  }
  ...
  def trainOneEpoch(epoch: Int): Unit = {
    ...
    LOG.info("epoch " + epoch + " llh " + loglikelihood())
  }
  ...
  def loglikelihood(): Double = {
    ...
    val numPerTask = Math.ceil(lDAModel.V * 1.0 / taskNum).toInt
    val start = numPerTask * taskId
    val end   = Math.min(lDAModel.V, numPerTask * (taskNum + 1))
