...
/**
  ...
  */
private[tree] object RandomForest {

  private val LOG = LogFactory.getLog("RandomForest")

  /**
    * Train a random forest.
    *
    * @param trainData Training data: Array of `LabeledData`
    * @param validData Validation data: Array of `LabeledData`
  ...
  def train(
    ...
    LOG.info("numExamples: " + metadata.numExamples)

    // Find the splits and the corresponding bins (interval between the splits) using a sample
    // of the input data.
    timer.start("findSplits")
    val splits = findSplits(trainData, metadata, seed)
    timer.stop("findSplits")
//    LOG.info("numBins: feature: number of bins")
//    LOG.info(Range(0, metadata.numFeatures).map { featureIndex =>
...
//    }.mkString("\n"))
    ...
    LOG.info("max memory usage for aggregates = " + maxMemoryUsage + " bytes.")
    ...
    LOG.info("Internal timing for DecisionTree:")
    LOG.info(s"$timer")
  ...
  private[tree] def findBestSplits(
    ...
    LOG.info("numFeatures = " + metadata.numFeatures)
    LOG.info("numClasses = " + metadata.numClasses)
    LOG.info("isMulticlass = " + metadata.isMulticlass)
    LOG.info("isMulticlassWithCategoricalFeatures = " +
      metadata.isMulticlassWithCategoricalFeatures)
    LOG.info("using nodeIdCache = " + nodeIdCache.nonEmpty.toString)

    /*
     * Performs a sequential aggregation over a partition for a particular tree and node.
     * For each feature, the aggregate sufficient statistics are updated for the relevant bins.
     *
     * @param treeIndex Index of the tree that we want to perform aggregation for.
     * @param nodeInfo The node info for the tree node.
     * @param agg Array storing aggregate calculation, with a set of sufficient statistics for each (node, feature, bin).
    ...
    def nodeBinSeqOp(
      ...
      if (nodeInfo != null) {
        ...
        // Unordered features in categorical features
        // Satisfy: 2 * ((1 << numCategories - 1) - 1) <= maxBins
    ...
    // After a `reduceByKey` operation, stats of a node will be shuffled to a particular partition and be combined together,
    ...
    val nodeStatsAggregators = Array.tabulate(numNodes) { nodeIndex =>
      nodeToFeatures match {
        case Some(s) => new DTStatsAggregator(metadata, s.get(nodeIndex))
        case None => new DTStatsAggregator(metadata, None)
      }
    }

    if (nodeIdCache.nonEmpty) {
      // iterator all instances in current partition and update aggregate stats
      input.zip(nodeIdCache.get.nodeIdsForInstances).map{ binSeqOpWithNodeIdCache(nodeStatsAggregators, _) }
    } else {
      // iterator all instances in current partition and update aggregate stats
      input.map{ binSeqOp(nodeStatsAggregators, _) }
    }

    val nodeToBestSplits = nodeStatsAggregators.zipWithIndex.map {
      case (aggStats, nodeIndex) =>
        //LOG.info(s"agg stats of node: node[$nodeIndex] ${aggStats.getAllStats().mkString(",")}")
        val featuresForNode = nodeToFeatures match {
          case Some(s) => s.get(nodeIndex)
          case None => None
        }
    ...
    nodesForGroup.foreach { case (treeIndex, nodesForTree) =>
      nodesForTree.foreach { node =>
        ...
        LOG.info(s"best split: " +
          s"tree[$treeIndex] " +
          s"node[$nodeIndex] " +
          s"feature[${split.toOld.feature}] " +
          s"threshold[${split.toOld.threshold}] " +
          s"type[${split.toOld.featureType}] " +
          s"category[${split.toOld.categories.mkString(",")}]")
        ...
        node.isLeaf = isLeaf
        node.stats = stats
        LOG.info(s"Node: tree[$treeIndex] node[${node.id}]")
        ...
        if (!isLeaf) {
          ...
          LOG.info(s"leftChild: tree[$treeIndex] " +
            s"nodeIndex[${node.leftChild.get.id}] " +
            s"impurity[${stats.leftImpurity}]")
          LOG.info(s"rightChild: tree[$treeIndex] " +
            s"nodeIndex[${node.rightChild.get.id}] " +
  ...
  private def calculateImpurityStats(
    ...
    val impurity: Float = if (stats == null) {
    ...
    val leftWeight = leftCount / totalCount.toFloat
    val rightWeight = rightCount / totalCount.toFloat
    ...
    val splitsAndImpurityInfo =
      validFeatureSplits.map { case (featureIndexIdx, featureIndex) =>
        ...
        if (binAggregates.metadata.isContinuous(featureIndex)) {
          ...
          LOG.info("Centroids for categorical variable: " + centroidForCategories.mkString(","))

          // bins sorted by centroids
          val categoriesSortedByCentroid = centroidForCategories.toList.sortBy(_._2)

          LOG.info("Sorted centroids for categorical variable = " +
  ...
  /**
    ...
    *
    * @param input Training data: Array of [[LabeledData]]
    * @param metadata Learning and dataset metadata
    * @param seed random seed
    * @return Splits, an Array of [[Split]] of size (numFeatures, numSplits)
    */
  protected[tree] def findSplits(
    ...
    val sampledInput = if (continuousFeatures.nonEmpty) {
      ...
      LOG.info("fraction of data used for calculating quantiles = " + fraction)
      Random.shuffle((0 to metadata.numExamples).toList).take((fraction * metadata.numExamples).toInt)
    } else {
      Array.empty[Int]
    }
    */

    findSplitsBySorting(input, metadata, continuousFeatures)
  }
  ...
  private def findSplitsBySorting(
    ...
    val continuousSplits: scala.collection.Map[Int, Array[Split]] = {
      input.flatMap { point =>
          continuousFeatures.map(idx => (idx, point.getX.asInstanceOf[IntFloatVector].get(idx))).filter(_._2 != 0.0)
        ...
        .map { case (idx, samples) =>
          ...
          //LOG.info(s"featureIndex = $idx, numSplits = ${splits.length}")
          (idx, splits)
        }
    }
  ...
  private[tree] def extractMultiClassCategories(
                                                 input: Int,
                                                 maxFeatureValue: Int): List[Float] = {
    var categories = List[Float]()
    var j = 0
    var bitShiftedInput = input
    while (j < maxFeatureValue) {
      if (bitShiftedInput % 2 != 0) {
        // updating the list of categories.
        categories = j.toFloat :: categories
      }
      // Right shift by one
  ...
  private[tree] def findSplitsForContinuousFeature(
                                                    featureSamples: Iterable[Float],
                                                    metadata: DecisionTreeMetadata,
                                                    featureIndex: Int): Array[Float] = {
    require(metadata.isContinuous(featureIndex),
      "findSplitsForContinuousFeature can only be used to find splits for a continuous feature.")

    val splits: Array[Float] = if (featureSamples.isEmpty) {
      Array.empty[Float]
    } else {
      ...
      val partValueCountMap = scala.collection.mutable.Map[Float, Int]()
      ...
      val valueCountMap: Map[Float, Int] = if (numSamples - partNumSamples > 0) {
        partValueCountMap.toMap + (0.0f -> (numSamples - partNumSamples))
      } else {
      ...
      if (possibleSplits == 0) {
        ...
        (1 to possibleSplits)
          .map(index => (valueCounts(index - 1)._1 + valueCounts(index)._1) / 2.0f)
          .toArray
      } else {
        // stride between splits
        val stride: Float = numSamples.toFloat / (numSplits + 1)
        ...
        while (index < valueCounts.length) {
          ...
          if (previousGap < currentGap) {
            splitsBuilder += (valueCounts(index - 1)._1 + valueCounts(index)._1) / 2.0f
            targetCount += stride
          }
          index += 1
        }
  ...
  private[tree] def selectNodesToSplit(
    ...
    if (memUsage > maxMemoryUsage) {
      ...
      LOG.info(s"Tree learning is using approximately $memUsage bytes per iteration, which" +
