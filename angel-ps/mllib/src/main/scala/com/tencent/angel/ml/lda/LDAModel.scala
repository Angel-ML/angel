...
class LDAModel(conf: Configuration, _ctx: TaskContext = null) extends OldMLModel(conf, _ctx) {
  ...
  val threadNum = conf.getInt(ANGEL_WORKER_THREAD_NUM, 2)
  ...
  def blockNum(V: Int, K: Int): Int = {
    val blockNum = (10 * 1024 * 1024 / (K * 4))
    return Math.max(1, Math.max(V / 20000, Math.min(V / psNum, blockNum)))
  }
      ...
      while (!finish) {
        in.readLine() match {
          case line: String =>
            ...
            val vector = VFactory.sparseIntVector(K)
            for (i <- 1 until parts.length) {
              val pp = parts(i).split(":")
              val (kk, vv) = (pp(0).toInt, pp(1).toInt)
              vector.set(kk, vv)
              update.set(kk, vv + update.get(kk))
            }
            wtMat.getClient.increment(topic, vector, false)
