...
class GBDTLearner(override val ctx: TaskContext) extends MLLearner(ctx) {
  ...
  def initParam(): Unit = {
    ...
    param.maxDepth = conf.getInt(MLConf.ML_TREE_MAX_DEPTH, MLConf.DEFAULT_ML_TREE_MAX_DEPTH)
    param.colSample = conf.getFloat(MLConf.ML_TREE_FEATURE_SAMPLE_RATE, MLConf.DEFAULT_ML_TREE_FEATURE_SAMPLE_RATE)
    param.learningRate = conf.getFloat(MLConf.ML_LEARN_RATE, MLConf.DEFAULT_ML_LEARN_RATE.asInstanceOf[Float])
    param.maxThreadNum = conf.getInt(MLConf.ML_TREE_NUM_THREAD, MLConf.DEFAULT_ML_TREE_NUM_THREAD)
