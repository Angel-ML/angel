...
public class MixedBinaryIntersectionExecutor {
  ...
  private static String IntKeyDummyFunc = "{\n" +
		  ...
		  " 		if (part.isDense()) {\n" +
		  " 			CommonFuncs.getInstance().CompDenseVsDummy(part, v2, resPart, base, op);\n" +
		  " 		} else if (part.isSparse()) {\n" +
		  "  			CommonFuncs.getInstance().CompSparseVsOther(part, v2, resPart, base, op);\n" +
		  " 		} else { // sorted\n" +
		  " 			if (op.isKeepStorage()){\n" +
		  " 				  CommonFuncs.getInstance().CompSortedVsOther(part, v2, resPart, base, op);\n" +
		  " 			} else {\n" +
		  ...
		  try {
			...
			for (int i = 0; i < itypes.length; i++) {
			  for (int j = 0; j < dtypes.length; j++) {
				declare = declTemplate.replaceAll("&ReturnType", "Vector")
						.replaceAll("&Parameters", "Comp&itpye&dtype1Vector v1, &itpyeDummyVector v2, Binary op")
						.replaceAll("&itpye", itypes[i])
						.replaceAll("&dtype1", dtypes[j]);
				if (itypes[i] == "Int") {
				  body = IntKeyDummyFunc;
				} else {
				  body = LongKeyDummyFunc;
				}
			...
			executor = (Executor) ee.createFastClassBodyEvaluator(
					new Scanner(null, new StringReader(
							template)),
					"MixedBinaryIntersectionExecutor",
					null,
