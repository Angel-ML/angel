...
public class LongLongSortedVectorStorage implements LongLongVectorStorage {
  ...
  public long get(long idx) {
	if (idx < 0 || idx > dim - 1) {
	  ...
	  int i = Arrays.binarySearch(indices, 0, size, idx);
  ...
  public void set(long idx, long value) {
	...
	if (size == 0 || idx < indices[0]) {
	  ...
	  point = Arrays.binarySearch(indices, 0, size, idx);
	  if (point >= 0) {
		values[point] = value;
		return;
	  } else {
  ...
  public LongLongVectorStorage clone() {
	return new LongLongSortedVectorStorage(dim, size, ArrayCopy.copy(indices), ArrayCopy.copy(values));
  }
  ...
  public boolean hasKey(long key) {
	return (size != 0 && key <= indices[size - 1] && key >= indices[0] &&
			Arrays.binarySearch(indices, 0, size, key) > 0);
  }
