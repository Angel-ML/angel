...
class Signum(inplace: Boolean) extends MapFunc {
  def this() = this(false)
  ...
  override def apply(elem: Double): Double = Math.signum(elem)

  override def apply(elem: Float): Float = Math.signum(elem)

  override def apply(elem: Long): Long = Math.signum(elem).toLong

  override def apply(elem: Int): Int = Math.signum(elem).toInt
