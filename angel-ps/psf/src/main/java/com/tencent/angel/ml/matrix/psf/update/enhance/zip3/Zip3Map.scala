...
/**
  ...
  */
class Zip3Map(param: MFUpdateParam) extends MFUpdateFunc(param) {
  def this(matrixId: Int, fromId1: Int, fromId2: Int, fromId3: Int, toId: Int, func: Zip3MapFunc) =
  ...
  private def doUpdate(rows: Array[ServerLongDoubleRow], func: Serialize): Unit = {
    ...
    while (iter.hasNext) {
      ...
      entry.setValue(mapper.call(from1.get(key), from2.get(key), from3.get(key)))
    }
    rows(3).setSplit(to)
  }
  ...
  private def doUpdate(rows: Array[ServerLongFloatRow], func: Serialize): Unit = {
    ...
    while (iter.hasNext) {
      val entry = iter.next
      val key = entry.getLongKey
      entry.setValue(mapper.call(from1.get(key), from2.get(key), from3.get(key)).toFloat)
    }
    rows(3).setSplit(to)
  }
}
