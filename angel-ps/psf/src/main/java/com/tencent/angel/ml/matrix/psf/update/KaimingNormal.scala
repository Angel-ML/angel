...
class KaimingNormal(param: MMUpdateParam) extends MMUpdateFunc(param) {

  def this(matrixId: Int, rowId: Int, gain: Double, fin: Long) =
    this(new MMUpdateParam(matrixId, Array[Int](rowId), Array[Double](gain * math.sqrt(2.0 / fin))))

  def this(matrixId: Int, startId: Int, length: Int, gain: Double, fin: Long) =
    this(new MMUpdateParam(matrixId, startId, length, Array[Double](gain * math.sqrt(2.0 / fin))))
      ...
      case r: ServerIntFloatRow =>
        r.elemUpdate(new FloatElemUpdateFunc {
          override def update(): Float = {
            (math.sqrt(a) * rand.nextGaussian()).toFloat
          }
        })
