...
class Log10(inplace: Boolean) extends MapFunc {
  def this() = this(false)
  ...
  override def apply(elem: Double): Double = Math.log10(elem)

  override def apply(elem: Float): Float = Math.log10(elem).toFloat

  override def apply(elem: Long): Long = Math.log10(elem).toLong

  override def apply(elem: Int): Int = Math.log10(elem).toInt
