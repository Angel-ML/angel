...
public class Utils {
  ...
  public static ArrayList<Map.Entry<ServerRow, ArrayList<Integer>>> pick(ServerRow[] serverRows,
    Long[] rows, Long[] cols, Double[] values) {
    ...
    for (ServerRow row : serverRows) {
      ...
      Map.Entry<ServerRow, ArrayList<Integer>> pair =
        new java.util.AbstractMap.SimpleEntry<>(row, indics);
      result.add(pair);
    }
