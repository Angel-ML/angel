...
object JsonUtils {
  ...
  def extract[T](jast: JValue, key: String, default: Option[T] = None): Option[T] = {
    jast \ key match {
      case JNothing => default
      case v: JValue => Some(v.extract[T])
    }
  }
  ...
  def fillGraph(jsonAst: JValue)(implicit graph: Graph): Unit = {
    ...
    val layerList = jsonAst \ ParamKeys.layers match {
      ...
      case _ => throw MLException("there is no layer conf")
    }
