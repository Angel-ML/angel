  ...
  def build(): Optimizer = {
    name.trim.toLowerCase match {
      case "sgd" =>
        ...
        opt
      case _ => throw MLException(s"Unknown Optimizer $name !")
    }
  }
}
...
object OptParams {
  ...
  def apply(json: JValue, default: Option[OptParams] = None): OptParams = {
    json match {
      ...
      case jast: JString => throw MLException(s"No such a optimizer: ${jast.extract[String]}!")
      case _: JValue => // for dictionary fashion
        ...
        optType.toLowerCase match {
          ...
          case _ => throw MLException(s"No such a optimizer: $optType!")
        }
    }
  }
}
