...
object LayerParams {
  ...
  def apply(json: JValue,
    ...
    val layerType: String = json \ ParamKeys.typeName match {
      case JNothing => throw MLException("type in Layer is not set!")
    ...
    layerType.toLowerCase match {
      case `simpleInput` =>
        val outputDim: Int = json \ ParamKeys.outputDim match {
          case JNothing => throw MLException("outputDim in InputLayer is not set!")
      ...
      case `embedding` =>
        val outputDim: Int = json \ ParamKeys.outputDim match {
          case JNothing => throw MLException("outputdim in EmbeddingLayer is not set!")
          case v: JInt => v.extract[Int]
        }

        val numFactors = json \ ParamKeys.numFactors match {
          case JNothing => throw MLException("numfactors in EmbeddingLayer is not set!")
          case v: JInt => v.extract[Int]
        }
      ...
      case `simpleLoss` =>
        val inputLayer = json \ ParamKeys.inputLayer match {
          case JNothing => throw MLException("inputlayer in LossLayer is not set!")
      ...
      case `concat` | `dotPooling` | `mulPooling` | `sumPooling` =>
        val outputDim = json \ ParamKeys.outputDim match {
          case JNothing => throw MLException("outputDim in JoinLayer is not set!")
          case opd: JValue => opd.extract[Int]
        }

        val inputLayers = json \ ParamKeys.inputLayers match {
          case JNothing => throw MLException("inputLayers in JoinLayer is not set!")
      ...
      case `biInnerCross` | `biInnerSumCross` | `biInteractionCross` | `biOuterCross` =>
        ...
        val inputLayer = json \ ParamKeys.inputLayer match {
          case JNothing => throw MLException("inputLayer in CrossLayer is not set!")
          case il: JValue => il.extract[String]
        }
      ...
      case `fcLayer` =>
        val outputDim = json \ ParamKeys.outputDims match {
          case JNothing => throw MLException("outputDims in FCLayer is not set!")
          case opd: JValue => opd.extract[Array[Int]]
        }

        val inputLayer = json \ ParamKeys.inputLayer match {
          case JNothing => throw MLException("inputLayer in FCLayer is not set!")
          case il: JValue => il.extract[String]
        }

        val transParams = json \ ParamKeys.transFuncs match {
          case JNothing => throw MLException("transFuncs in FCLayer is not set!")
