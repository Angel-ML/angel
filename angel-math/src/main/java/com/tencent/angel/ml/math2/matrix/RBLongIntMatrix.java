...
<<<<<<< HEAD:angel-math/src/main/java/com/tencent/angel/ml/math2/matrix/RBLongIntMatrix.java
import com.tencent.angel.ml.math2.MFactory;
=======
>>>>>>> hotfix:angel-ps/core/src/main/java/com/tencent/angel/ml/math2/matrix/RBLongIntMatrix.java
...
public class RBLongIntMatrix extends RowBasedMatrix<LongIntVector> {

  public RBLongIntMatrix() {
    super();
  }

  public RBLongIntMatrix(int matrixId, int clock, LongIntVector[] rows) {
    super(matrixId, clock, rows[0].getDim(), rows);
  }

  public RBLongIntMatrix(LongIntVector[] rows) {
    this(0, 0, rows);
  }

  public RBLongIntMatrix(int matrixId, int clock, int numRows, long numCols) {
  ...
  public RBLongIntMatrix(int numRows, long numCols) {
  ...
  public RowBasedMatrix calulate(int rowId, Vector other, Binary op) {
    ...
    if (op.isInplace()) {
      ...
      res = new RBLongIntMatrix(matrixId, clock, rows.length, (long) cols);
    }
    ...
    if (op.isInplace()) {
      ...
      for (int i = 0; i < rows.length; i++) {
        if (i == rowId) {
          res.setRow(rowId, (LongIntVector) BinaryExecutor.apply(rows[rowId], other, op));
        } else if (rows[i] != null) {
  ...
  public RowBasedMatrix calulate(Matrix other, Binary op) {
    ...
    if (op.isInplace()) {
      ...
      LongIntVector[] outRows = new LongIntVector[rows.length];
      for (int i = 0; i < rows.length; i++) {
        if (null == rows[i]) {
          initEmpty(i);
        }
        ...
        outRows[i] =
            (LongIntVector) BinaryExecutor.apply(rows[i], ((RowBasedMatrix) other).rows[i], op);
      }
      return new RBLongIntMatrix(matrixId, clock, outRows);
    }
  }
  ...
  public RowBasedMatrix calulate(Unary op) {
    if (op.isInplace()) {
      ...
      for (int i = 0; i < rows.length; i++) {
        ...
        outRows[i] = (LongIntVector) UnaryExecutor.apply(rows[i], op);
      }
      return new RBLongIntMatrix(matrixId, clock, outRows);
    }
  }
  ...
  @Override
  public void setRow(int idx, LongIntVector v) {
    assert cols == v.getDim();
    rows[idx] = v;
  }

  @Override
  public void setRows(LongIntVector[] rows) {
    for (LongIntVector v : rows) {
      assert cols == v.getDim();
    }
    this.rows = rows;
  }
  ...
  @Override
  public Matrix copy() {
    LongIntVector[] newRows = new LongIntVector[rows.length];
    for (int i = 0; i < rows.length; i++) {
      newRows[i] = rows[i].copy();
    }
    return new RBLongIntMatrix(matrixId, clock, newRows);
  }
